...
const news = [
  ...
  },
  {
    ...
    date: '5th Jan 2020',
    image: './images/blue_top_follow_through.JPG',
...
const newsTitleElements = document.querySelectorAll('.news-title');
const newsDateElements = document.querySelectorAll('.news-date');
const newsImageElements = document.querySelectorAll('.news-image');
const newsBodyElements = document.querySelectorAll('.news-body');

function populateNews() {
  for(let i = 0; i < news.length; i++) {
    if(newsTitleElements[i]) {
      newsTitleElements[i].innerText = news[i].title;
    }
    if(newsDateElements[i]) {
      newsDateElements[i].innerText = news[i].date;
    }
    if(newsImageElements[i]) {
      newsImageElements[i].src = news[i].image;
    }
    if(newsBodyElements[i]) {
      newsBodyElements[i].innerText = news[i].body;
    }
  }
};
